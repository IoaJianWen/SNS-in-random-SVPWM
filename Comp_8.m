%% 
% GAP-RP-SVPWM 线电压信号波形与功率谱 三相电流信号波形与功率谱
...
while (T_duty_A_gap < 4/f0)
    ...
    % GAP_RP_SVPWM
    if ite_A_gap == 1
        R_A_gap(ite_A_gap) = (1 - duty_A_gap(ite_A_gap)) / 2;
    else
        k_min = ceil(fx/fs * (1 - R_A_gap(ite_A_gap-1) + duty_A_gap(ite_A_gap)));
        k_max = floor(fx/fs * (2 - R_A_gap(ite_A_gap-1)));
        if k_min <= k_max
            k = randi([k_min k_max]);
            R_A_gap(ite_A_gap) = k*fs/fx + R_A_gap(ite_A_gap-1) - duty_A_gap(ite_A_gap) - 1;
        else
            R_A_gap(ite_A_gap) = (1 - duty_A_gap(ite_A_gap)) / 2;
        end
    end
    ...
    pulsewidth = (1/fs) * duty_A_gap(ite_A_gap);
    
    T_ss = 0:1/fss:(1/fs - 1/fss);
    rect = (0:1/fs:1/fs)'; % 一次循环生成一个 该时刻占空比和fs 对应下的脉冲
    Y_1 = pulstran(T_ss - (R_A_gap(ite_A_gap) + duty_A_gap(ite_A_gap) / 2) / fs,rect,...
        'rectpuls',pulsewidth);
    S_A_gap = [S_A_gap,Y_1];
    
    T_duty_A_gap = T_duty_A_gap + 1/fs; % 占空比的采样间隔（时间）
...
while (T_duty_B_gap < 4/f0)
    ...
    % GAP_RP_SVPWM
    if ite_B_gap == 1
        R_B_gap(ite_B_gap) = (1 - duty_B_gap(ite_B_gap)) / 2;
    else
        k_min = ceil(fx/fs * (1 - R_B_gap(ite_B_gap-1) + duty_B_gap(ite_B_gap)));
        k_max = floor(fx/fs * (2 - R_B_gap(ite_B_gap-1)));
        if k_min <= k_max
            k = randi([k_min k_max]);
            R_B_gap(ite_B_gap) = k*fs/fx + R_B_gap(ite_B_gap-1) - duty_B_gap(ite_B_gap) - 1;
        else
            R_B_gap(ite_B_gap) = (1 - duty_B_gap(ite_B_gap)) / 2;
        end
    end
    ...
    pulsewidth = (1/fs) * duty_B_gap(ite_B_gap);
    
    T_ss = 0:1/fss:(1/fs - 1/fss);
    rect = (0:1/fs:1/fs)'; % 一次循环生成一个 该时刻占空比和fs 对应下的脉冲
    Y_2 = pulstran(T_ss - (R_B_gap(ite_B_gap) + duty_B_gap(ite_B_gap) / 2) / fs,rect,...
        'rectpuls',pulsewidth);
    S_B_gap = [S_B_gap,Y_2];
    
    T_duty_B_gap = T_duty_B_gap + 1/fs; % 占空比的采样间隔（时间）
...
while (T_duty_C_gap < 4/f0)
    ...
    % GAP_RP_SVPWM
    if ite_C_gap == 1
        R_C_gap(ite_C_gap) = (1 - duty_C_gap(ite_C_gap)) / 2;
    else
        k_min = ceil(fx/fs * (1 - R_C_gap(ite_C_gap-1) + duty_C_gap(ite_C_gap)));
        k_max = floor(fx/fs * (2 - R_C_gap(ite_C_gap-1)));
        if k_min <= k_max
            k = randi([k_min k_max]);
            R_C_gap(ite_C_gap) = k*fs/fx + R_C_gap(ite_C_gap-1) - duty_C_gap(ite_C_gap) - 1;
        else
            R_C_gap(ite_C_gap) = (1 - duty_C_gap(ite_C_gap)) / 2;
        end
    end
    ...
    pulsewidth = (1/fs) * duty_C_gap(ite_C_gap);
    
    T_ss = 0:1/fss:(1/fs - 1/fss);
    rect = (0:1/fs:1/fs)'; % 一次循环生成一个 该时刻占空比和fs 对应下的脉冲
    Y_3 = pulstran(T_ss - (R_C_gap(ite_C_gap) + duty_C_gap(ite_C_gap) / 2) / fs,rect,...
        'rectpuls',pulsewidth);
    S_C_gap = [S_C_gap,Y_3];
    
    T_duty_C_gap = T_duty_C_gap + 1/fs; % 占空比的采样间隔（时间）
...
% RP-SVPWM 线电压信号波形与功率谱 三相电流信号波形与功率谱
...
while (T_duty_A < 4/f0)
    ...
    T_ss = 0:1/fss:(1/fs - 1/fss);
    rect = (0:1/fs:1/fs)'; % 一次循环生成一个 该时刻占空比和fs 对应下的脉冲
    Y_1 = pulstran(T_ss - (R_A(ite_A) + duty_A(ite_A) / 2) / fs,rect,...
        'rectpuls',pulsewidth);
    S_A = [S_A,Y_1];
...
while (T_duty_B < 4/f0)
    ...
    T_ss = 0:1/fss:(1/fs - 1/fss);
    rect = (0:1/fs:1/fs)'; % 一次循环生成一个 该时刻占空比和fs 对应下的脉冲
    Y_2 = pulstran(T_ss - (R_B(ite_B) + duty_B(ite_B) / 2) / fs,rect,...
        'rectpuls',pulsewidth);
    S_B = [S_B,Y_2];
...
while (T_duty_C < 4/f0)
    ...
    T_ss = 0:1/fss:(1/fs - 1/fss);
    rect = (0:1/fs:1/fs)'; % 一次循环生成一个 该时刻占空比和fs 对应下的脉冲
    Y_3 = pulstran(T_ss - (R_C(ite_C) + duty_C(ite_C) / 2) / fs,rect,...
        'rectpuls',pulsewidth);
    S_C = [S_C,Y_3];
