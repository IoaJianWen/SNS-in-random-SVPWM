...
while (T_duty_A < 4/f0)
    ...
    % GAP_RP_SVPWM
    if ite_A == 1
        R_A(ite_A) = (1 - duty_A(ite_A)) / 2;
    else
        k_min = ceil(fx/fs * (1 - R_A(ite_A-1) + duty_A(ite_A)));
        k_max = floor(fx/fs * (2 - R_A(ite_A-1)));
        if k_min <= k_max
            k = randi([k_min k_max]);
            R_A(ite_A) = k*fs/fx + R_A(ite_A-1) - duty_A(ite_A) - 1;
        else
            R_A(ite_A) = (1 - duty_A(ite_A)) / 2;
        end
    end
...
while (T_duty_B < 4/f0)
    ...
    % GAP_RP_SVPWM
    if ite_B == 1
        R_B(ite_B) = (1 - duty_B(ite_B)) / 2;
    else
        k_min = ceil(fx/fs * (1 - R_B(ite_B-1) + duty_B(ite_B)));
        k_max = floor(fx/fs * (2 - R_B(ite_B-1)));
        if k_min <= k_max
            k = randi([k_min k_max]);
            R_B(ite_B) = k*fs/fx + R_B(ite_B-1) - duty_B(ite_B) - 1;
        else
            R_B(ite_B) = (1 - duty_B(ite_B)) / 2;
        end
    end
...
while (T_duty_C < 4/f0)
    ...
    % GAP_RP_SVPWM
    if ite_C == 1
        R_C(ite_C) = (1 - duty_C(ite_C)) / 2;
    else
        k_min = ceil(fx/fs * (1 - R_C(ite_C-1) + duty_C(ite_C)));
        k_max = floor(fx/fs * (2 - R_C(ite_C-1)));
        if k_min <= k_max
            k = randi([k_min k_max]);
            R_C(ite_C) = k*fs/fx + R_C(ite_C-1) - duty_C(ite_C) - 1;
        else
            R_C(ite_C) = (1 - duty_C(ite_C)) / 2;
        end
    end
